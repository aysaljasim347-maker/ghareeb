...
-- File: migrations/003_schema_v2_fixes.sql
...
-- NOTE: Full RLS enforcement requires the app pool to connect as 'app_user'
-- (not 'disasteraid_user'). Create roles separately:
--   CREATE ROLE app_user   NOINHERIT LOGIN PASSWORD '...';
--   CREATE ROLE admin_user NOINHERIT LOGIN PASSWORD '...' BYPASSRLS;
--   GRANT CONNECT ON DATABASE disasteraid TO app_user, admin_user;
...
-- ── [C3] campaign_financials view ───────────────────────────
CREATE OR REPLACE VIEW campaign_financials AS
SELECT
    c.id                                                                   AS campaign_id,
    c.title,
    c.goal_pkr,
    c.raised_pkr                                                           AS raised_cached,
    c.spent_pkr                                                            AS spent_cached,
    COALESCE(SUM(d.amount_pkr) FILTER (WHERE d.status = 'CONFIRMED'), 0)  AS raised_actual,
    COALESCE(SUM(t.budget_pkr) FILTER (WHERE t.status = 'PAID'),      0)  AS spent_actual,
    c.raised_pkr - COALESCE(SUM(d.amount_pkr) FILTER (WHERE d.status = 'CONFIRMED'), 0) AS raised_drift,
    c.spent_pkr  - COALESCE(SUM(t.budget_pkr) FILTER (WHERE t.status = 'PAID'),      0) AS spent_drift
FROM campaigns c
LEFT JOIN donations d ON d.campaign_id = c.id
LEFT JOIN tasks     t ON t.campaign_id = c.id
GROUP BY c.id;

COMMENT ON VIEW campaign_financials IS
    'Use raised_drift / spent_drift in a scheduled job to detect counter corruption. Both should always be 0.';

-- ── Seed system_config defaults if missing ───────────────────
INSERT INTO system_config (key, value) VALUES
    ('disaster_mode',      'false'::jsonb),
    ('system_state',       '"NORMAL"'::jsonb),
    ('max_task_radius_km', '50'::jsonb)
