...
-- ── ENUM Types ──────────────────────────────────────────────
CREATE TYPE user_role AS ENUM ('DONOR', 'BENEFICIARY', 'VOLUNTEER', 'NGO', 'COORDINATOR', 'ADMIN');
CREATE TYPE task_status AS ENUM ('OPEN', 'ASSIGNED', 'CLAIMED', 'IN_PROGRESS', 'SUBMITTED', 'COORDINATOR_VERIFIED', 'PAID', 'FLAGGED', 'CANCELLED');
CREATE TYPE task_urgency AS ENUM ('LOW', 'MEDIUM', 'HIGH', 'CRITICAL');
CREATE TYPE task_source AS ENUM ('BENEFICIARY_REQUEST', 'NGO_CAMPAIGN', 'PLATFORM_CAMPAIGN', 'ADMIN_CREATED');
CREATE TYPE task_event_type AS ENUM ('CREATED', 'SEEN', 'ASSIGNED', 'CLAIMED', 'STARTED', 'SUBMITTED', 'VERIFIED', 'PAID', 'FLAGGED', 'CHAT_STARTED', 'UPDATED', 'CANCELLED');
CREATE TYPE campaign_status AS ENUM ('DRAFT', 'PENDING_APPROVAL', 'ACTIVE', 'COMPLETED', 'REJECTED');
...
CREATE TABLE campaigns (
    id          SERIAL PRIMARY KEY,
    ngo_id      INT REFERENCES ngo_profiles(id) ON DELETE SET NULL,
...
CREATE INDEX idx_tasks_status          ON tasks(status) WHERE status IN ('OPEN', 'ASSIGNED');
CREATE INDEX idx_tasks_claimed         ON tasks(claimed_by) WHERE claimed_by IS NOT NULL;
...
INSERT INTO roles (name) VALUES
    ('DONOR'), ('BENEFICIARY'), ('VOLUNTEER'), ('NGO'), ('COORDINATOR'), ('ADMIN');

INSERT INTO app_config (id) VALUES (1);

-- Admin user (password should be changed on first login)
INSERT INTO users (email, password_hash, role_id, name) VALUES
    ('admin@disasteraid.pk', '$2b$10$3euPcmQFCiblsZeEu5s7p.9wWHJPaYbmo9gq3g5l6h8d5k8j5', 6, 'System Admin');
